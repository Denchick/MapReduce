...
class TestUtils(unittest.TestCase):
    ...
    def test_get_next_data_piece(self):
        def pattern_for_testing_without_exceptions(data,
                                                   separator,
                                                   count,
                                                   expected):
        ...
        def pattern_for_testing_with_exceptions(data,
                                                separator,
                                                count,
                                                error_type):
        ...
        pattern_for_testing_without_exceptions(
            ['kek', 'cheburek'], ';', 3, 'kek')
        pattern_for_testing_without_exceptions(
            ['kek', 'cheburek'], ';', 4, 'kek')
        pattern_for_testing_without_exceptions(
            ['kek', 'cheburek'], ';', 5, 'kek;cheburek')
        pattern_for_testing_without_exceptions(
            ['kek'], ';', 4, 'kek')
        pattern_for_testing_without_exceptions(
            ['kek', 'cheburek'], ';', 20, 'kek;cheburek')

        pattern_for_testing_with_exceptions(
            ['kek'], ';', '401', TypeError)
        pattern_for_testing_with_exceptions(
            ['kek'], True, 2, TypeError)
