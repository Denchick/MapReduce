...
class Piece:
    def __init__(self, index, data, directory):
        ...
        if os.path.exists(self.get_path(directory)):
            raise RuntimeWarning('File with {0} index already exists.'
                                 .format(self.index))
        self.write_to_filename(data, directory)

    def write_to_filename(self, data, directory):
        """ Запись данных в кусок. При этом, если в файле уже что-то было,
        ...
        if not os.path.isdir(directory):
            raise AttributeError("Directory path '{0}' is incorrect"
                                 .format(directory))
        if not isinstance(data, str):
            raise TypeError("Data must be a string but {0}"
                            .format(type(data), data))
    ...
    def get_data(self, directory):
        """ Чтение данных из куска с 0 байта и до коцна файла.
        Указатель на верхний элемент не сдвигается.
    ...
    def get_up_element(self, directory, separator):
        ...
        Читает строку в файле, начиная с указателя на верхний элемент.
        При этом, указатель не смещается.
    ...
    def move_data_pointer(self, offset):
        ...
        Raises:
            ...
            RuntimeError: если после смещения указатель на верхний элемент
            становится отрицательным.
        """
        if not isinstance(offset, int):
            raise TypeError("Offset must be an integer but {0}."
                            .format(type(offset), offset))
        if self.data_pointer + offset < 0:
            raise RuntimeError("Impossible to move on {0} bytes."
                               .format(offset))
    ...
    def get_path(self, directory):
        """ Формирует относительный путь до куска в виде: 'directory/index'
        в зависимости от ОС.
